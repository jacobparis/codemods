	...
	function resolveNode(node: Node): Node | undefined {
		...
		switch (node.getKind()) {
			...
			case SyntaxKind.ShorthandPropertyAssignment: {
				const shorthandAssignment = node as ShorthandPropertyAssignment
				const nameNode = shorthandAssignment.getNameNode() // Get the Identifier
				const definitionNodes = nameNode.getDefinitionNodes()
				if (definitionNodes.length > 0) {
					return resolveNode(definitionNodes[0]!) // Resolve the shorthand identifier definition
				}
				break
			}
...
export function resolveToIdentifier(symbolOrNode: Symbol | Node) {
	if (symbolOrNode instanceof Symbol) {
		const node = symbolOrNode.getDeclarations()[0]
		if (!node) {
			throw new Error('Node not found')
